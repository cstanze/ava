...
class Profile(Endpoint):
    ...
    def generate(self, avatars, text, usernames, kwargs):
        ...
        xp = kwargs.get('xp', '0')
        level = math.floor(int(xp) / 200)
        # Math.floor(xp / 200)
        next_xp = (int(level) + 1) * 200
        # next_xp = ((int(level) + 1) * 200) - int(xp)

        color = kwargs.get('color', 'grey')

        xp_dim = int((int(xp) - level * 100) / (next_xp - (level*100)) * 200) + 1, 5

        guildName = kwargs.get('guildname', 'No Guild?')
        eris = "{:,}".format(int(kwargs.get('eris', '0')))

        permsraw = kwargs.get('permsraw')
        prestige = kwargs.get('prestige', f'prestige{permsraw}')
        inventory = kwargs.get('inventory', '0 items worth 0 coins')

        ship = kwargs.get('ship', 'No ship')
        permsLevel = kwargs.get('permslevel', 'User')
        prefix = kwargs.get('prefix', 'a!')

        if prestige == 'prestige8.5':
            prestige = 'prestige9'
        ...
        draw.text((15, base.height - 85), f'PERMISSIONS LEVEL: {permsLevel} ({permsraw})\nLAST COMMAND: {prefix}profile\nSHIP NAME: {ship}', font=font2)
        ...
        draw.text((15, 365), f'{xp} XP - {next_xp} remaining', font=font2)

        draw.text((370, 290), 'Eris', font=font3)
        draw.text((370, 345), f'Balance For \n{guildName}:\n{eris} Eris', font=font2)
        ...
        if color == 'gay' or color == 'pride':
            ...
            colours = [(255, 255, 255), (255, 255, 255), (255, 255, 255), (255, 255, 255), (255, 255, 255), (255, 255, 255)]
