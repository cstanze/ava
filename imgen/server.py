...
def api(endpoint):
    ...
    e_r = endpoint_ratelimit(auth=request.headers.get('Authorization', ""), cache=cache, max_usage=max_usage)
    if e_r['X-RateLimit-Remaining'] == -1:
        ...
        return x
    # if endpoint == 'profile':
        # if request.headers.get('Authorization', "") != config.get('memer_token', None):
